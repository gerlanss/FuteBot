"""Diagnóstico v3: fixture completo com team IDs."""
import paramiko
...
# Fixtures com team IDs
conn = db._conn()
rows = conn.execute('''
    SELECT fixture_id, home_name, away_name, home_id, away_id, league_id, date, status
    FROM fixtures
...
for row in rows:
    fixture = {
        ...
        'teams': {
            'home': {'name': row['home_name'], 'id': row['home_id']},
            'away': {'name': row['away_name'], 'id': row['away_id']}
        }
    }
    try:
...
print(f'Jogos com previsao: {jogos_ok}/{len(rows)}')
print()
...
for prob_key, mercado_name in MERCADOS_CHECK:
    ...
    if ge60 == 0:
        bar = 'BLOQ: max {:.0%} < 60%'.format(mx)
    else:
        bar = f'OK ({ge60}/{len(vals)} passam 60%)'
    print(f'{mercado_name:<18} {len(vals):>3} {mn:>5.1%} {avg:>5.1%} {mx:>5.1%} {ge60:>5} {ge55:>5} {ge50:>5}  {bar}')
"
"""
